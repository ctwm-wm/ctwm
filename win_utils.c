...
/**
 * Move/resize/restack a window.
 *
 * This is effectively the backend for handling a ICCCM ConfigureWindow
 * request, but also backends other similar things that do some or all of
 * the same stuff.  A fair amount consideration and cross-pollinating
 * happens between the pieces of this, which makes it hard to cleanly
 * split up.
 *
 * In principle, this is our own defined operation, not X's.  In
 * practice, the XConfigureRequestEvent structure already contains all
 * the bits and flags we need for this and its consumers, so we [ab]use
 * it as our argument here.  We don't have a particular tie to it though;
 * creating our own structure to drive this func remains a considered
 * possibility.
 */
void
do_configure_window(TwmWindow *twm_win, const XConfigureRequestEvent *cre)
{
