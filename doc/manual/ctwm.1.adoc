...
Normally, key bindings refer to the KeyPress event, but with the "up" modifier
the binding refers to the KeyRelease event instead.
...
f.warpring `string`::
  ...
  by the argument `string`, which may be `"next"`, `"prev"`, `"startstack"`
  or `"finishstack"`) specified in the `WindowRing` variable.
